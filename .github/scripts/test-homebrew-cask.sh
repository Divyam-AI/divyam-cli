...
usage() {
    cat <<'EOF'
Usage:
  test-homebrew-cask.sh verify <renderer> <version> <arm64-archive> <x86_64-archive> <output-cask>
  test-homebrew-cask.sh local <renderer> <archive.tar.gz> <version>
EOF
}

verify_cask() {
    local renderer_path=${1:?usage: test-homebrew-cask.sh verify <renderer> <version> <arm64-archive> <x86_64-archive> <output-cask>}
    local version=${2:?usage: test-homebrew-cask.sh verify <renderer> <version> <arm64-archive> <x86_64-archive> <output-cask>}
    local arm64_archive=${3:?usage: test-homebrew-cask.sh verify <renderer> <version> <arm64-archive> <x86_64-archive> <output-cask>}
    local x86_64_archive=${4:?usage: test-homebrew-cask.sh verify <renderer> <version> <arm64-archive> <x86_64-archive> <output-cask>}
    local output_cask=${5:?usage: test-homebrew-cask.sh verify <renderer> <version> <arm64-archive> <x86_64-archive> <output-cask>}
...
smoke_local_cask() {
    local renderer_path=${1:?usage: test-homebrew-cask.sh local <renderer> <archive.tar.gz> <version>}
    local archive_path=${2:?usage: test-homebrew-cask.sh local <renderer> <archive.tar.gz> <version>}
    local version=${3:?usage: test-homebrew-cask.sh local <renderer> <archive.tar.gz> <version>}
...
PY
    ...
    local tap_root="$tmp_dir/homebrew-divyam-cli-cask-smoke-test"
    local initial_payload_root="$tmp_dir/initial-payload"
    ...
    for release_version in "$initial_version" "$version"; do
        local release_root="$web_root/releases/download/v$release_version"
        mkdir -p "$release_root"
        local arch
        for arch in arm64 x86_64; do
            local archive_name="divyam-cli-$release_version-darwin-$arch.tar.gz"
