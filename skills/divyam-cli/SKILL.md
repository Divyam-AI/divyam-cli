...
## First thing every session: confirm the active service account (once)

The first time this skill is used in a session, run:

```
divyam config current
```

Report the active service account in one line — e.g. *"Active service account: `<name>` (`<id>`)
on `<endpoint>`."* Resolve the name with `divyam sa get --id <saId>` if `config current` only
shows the id. Do this **once** and then proceed; don't re-announce it every turn. If no config is
active, say so and offer to help set one (`config use` / `config set`).
...
Secrets get their own handling — see below; a spoken provider key means you **hand off** the
...
**Inbound provider keys (`model-info create` / `update`).** Never run these yourself and never
echo the key back. `--provider-api-key` is an interactive option: build the whole command with
every non-secret flag and `--provider-api-key` as the **last, value-less flag**, then hand it to
the user to run in their own terminal, where the CLI prompts for the key with no echo:

```
divyam model-info create --provider-name google --api-type GEMINI \
  --model-names gemini-2.5-flash --provider-base-url "" --provider-api-key
# ^ run this in your terminal; it will prompt you to paste the key (no echo)
```

For a Vertex service-account JSON, use `--provider-api-key-file <path>` instead. If the user
pasted a key anyway, still route it through the hand-off command — don't inline it into argv.
...
1. Surface the `api_key` verbatim with a clear warning: *"Save this now — it's shown once and
   can't be retrieved later."*
2. **Offer to store it in a Divyam config profile** so it's persisted and usable. On yes, hand
   the user (or run, if they prefer) a `config set` with the token as an interactive bare flag,
   then activate it:
   ```
   ...
   endpoint/org/sa from `config current`.
3. Never write the key to the scratchpad, a file, or logs on your own initiative.
...
- Then **hand off** the command with `--provider-api-key` as a bare flag (secrets section).
...
switch to Bedrock with new prices). Restate the diff so they can catch a mistake before running.
...
then run. After promoting, offer the **A/B Traffic** dashboard link.
